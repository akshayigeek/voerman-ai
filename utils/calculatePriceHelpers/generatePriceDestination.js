...
const { verifyDestinationServices, verifyVolume } = require("./verifyDetails");
...
module.exports = async (emailLead, sql) => {
  try {
    ...
    const country_iso = destinationJSON.country_iso
      ? destinationJSON.country_iso
      : getCode(destinationJSON.country);

    const addressVerifyStatus = await verifyDestinationServices(destAddress);
